...
export default function ExamSettings() {
  ...
  async function publish() {
    ...
    setSaving(true)
    setError(null)
    // Publishing an exam automatically turns on its public link — the
      ...
      <form onSubmit={saveSettings} style={{ maxWidth: 480 }}>
        ...
        {atPublishLimit && (
          <p className="limit-note attn">
            You've published {usage?.published_exam_count} of {publishedLimit} exams allowed on your plan. <Link to="/billing">Upgrade to publish more →</Link>
          </p>
        )}
