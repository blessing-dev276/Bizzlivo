        ...
        <>
          <p style={{ marginTop: 24 }}>
            <Link to="/invites/assign" state={{ examId: exam.id, examTitle: exam.title }}>
              Assign this exam to members →
            </Link>
            {' · '}
            <Link to={`/exams/${exam.id}/roster`}>Assignment roster →</Link>
            {' · '}
            <Link to={`/exams/${exam.id}/analytics`}>View analytics →</Link>
          </p>

          <div className="public-link-panel">
            <div className="toggle-row" style={{ borderBottom: 'none', paddingBottom: 0 }}>
              <label style={{ margin: 0 }}>
                <strong style={{ color: 'var(--text-h)' }}>Public exam link</strong>
                <span style={{ fontWeight: 400, fontSize: 13, color: 'var(--text-dim)' }}>
                  Anyone with the link can take this exam — starting it registers them with your office right away.
                </span>
              </label>
              <input
                type="checkbox"
