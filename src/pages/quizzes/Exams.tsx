...
export default function Exams() {
  ...
  // One exam per resource — a resource already claimed by a quiz shouldn't
  // be offered again; the office should add more questions to that quiz instead.
  ...
  async function handleCreate(e: FormEvent) {
    ...
    try {
      ...
      const { data: exam, error: examError } = await supabase
        ...
        .single()
      if (examError) {
        if (examError.code === '23505') throw new Error('This resource already has a quiz — add more questions to that exam instead of creating a new one.')
        throw examError
      }
      ...
      navigate(`/quizzes/${exam.id}/generate`)
    } catch (err) {
  ...
  return (
    <div className="page">
      <div className="page-head list-header">
        <h1>Quizzes</h1>
        <button onClick={() => { resetNewExamForm(); setShowNew(true) }}>+ New quiz</button>
      </div>
                ...
                <label>
                  ...
                  {resources.length > availableResources.length && (
                    <p style={{ fontSize: 12.5, color: 'var(--text-faint)', marginTop: 6 }}>
                      Resources that already have a quiz aren't listed — add more questions to that quiz instead.
                    </p>
                  )}
                </label>
              )}
      ...
      {loading ? (
        ...
        <p>{rows.length === 0 ? 'No quizzes yet.' : 'No quizzes match this view.'}</p>
      ) : (
            ...
            <div className="exam-card" key={exam.id}>
              <div className="exam-top">
                <div className="exam-title-block">
                  <h3><Link to={`/quizzes/${exam.id}`}>{exam.title}</Link></h3>
              ...
              <div className="exam-meta">
                ...
                {exam.pendingCount > 0 && (
                  <>
                    ...
                    <span className="meta-div" />
                    <Link to={`/quizzes/${exam.id}/review`} className="review-cta">Review now →</Link>
                  </>
                )}
