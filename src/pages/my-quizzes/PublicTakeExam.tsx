...
export default function PublicTakeExam() {
  ...
  useEffect(() => {
    ...
    callFunction(`start-attempt?token=${encodeURIComponent(token)}`)
      ...
      .catch((err) => {
        if (cancelled) return
        setError(err instanceof Error ? err.message : 'This quiz link is not available.')
        setStage('error')
      })
    return () => {
      cancelled = true
  ...
  async function handleStart(e: FormEvent) {
    ...
    try {
      ...
      if (hasAccount) {
        ...
        if (!data.session) {
          // Registering by starting this quiz is itself proof of intent to
      ...
      <div style={{ position: 'relative', zIndex: 1, width: '100%', maxWidth: stage === 'taking' || stage === 'result' ? 640 : 380, margin: stage === 'taking' ? '40px 0' : 0 }}>
        ...
        {(stage === 'loading' || (stage === 'auth' && session)) && (
          <div className="auth-card">
            <p>{stage === 'auth' ? 'Starting your quiz…' : 'Loading quiz…'}</p>
