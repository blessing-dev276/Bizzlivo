...
export default function TakeExam() {
  ...
  const submit = useCallback(async () => {
    ...
    navigate(`/cbt/attempts/${attempt.id}/result`)
  ...
  useEffect(() => {
    ...
    async function init() {
      ...
      if (!settingsData) {
        setError('This exam has no settings configured yet.')
        setLoading(false)
        return
      }
      ...
      if (!currentAttempt) {
        ...
        if (settingsData.max_attempts !== 0 && pastAttempts >= settingsData.max_attempts) {
          setError("You've used all your attempts for this exam.")
          setLoading(false)
          return
        }
        ...
        if (selected.length === 0) {
          setError('This exam has no approved questions yet.')
          setLoading(false)
          return
        }
  ...
  if (loading) return <div className="page"><p>Loading exam…</p></div>
      ...
      <div className="page">
        <p className="form-error">
          {blocked.status === 'too_early'
            ? `This exam opens at ${blocked.when ? new Date(blocked.when).toLocaleString() : 'a later time'}.`
            : `This exam's window closed at ${blocked.when ? new Date(blocked.when).toLocaleString() : 'the scheduled time'} — it has been marked as missed.`}
        </p>
        <p><Link to="/cbt">Back to my exams</Link></p>
      </div>
    )
  }
