  ...
  return (
    <div className="page">
      ...
      {rows.length === 0 ? (
        ...
        <table>
          <thead>
            <tr><th>Exam</th><th>Due</th><th>Status</th><th></th></tr>
          </thead>
          <tbody>
            {rows.map(({ assignment, settings, attempts }) => {
              ...
              return (
                <tr key={assignment.id}>
                  ...
                  <td>
                    {status === 'in_progress' && <Link to={`/cbt/${assignment.id}/take`}>Resume →</Link>}
                    {status === 'not_started' && <Link to={`/cbt/${assignment.id}/take`}>Start →</Link>}
                    {(status === 'too_early' || status === 'missed') && <span style={{ color: 'var(--text-faint)' }}>—</span>}
                    {(status === 'passed' || status === 'failed') &&
                      (canAttemptAgain ? (
                        <Link to={`/cbt/${assignment.id}/take`}>Retake →</Link>
                      ) : (
                        <Link to={`/cbt/attempts/${attempt!.id}/result`}>View result →</Link>
                      ))}
                  </td>
                </tr>
              )
