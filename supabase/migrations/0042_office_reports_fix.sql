-- ============================================================
-- 0042 — fixes for the 0041 reporting RPCs.
--
--  * Every plpgsql reporting function gets `#variable_conflict use_column`
--    so identifiers like `rank_id` / `user_id` in embedded SQL resolve to
--    table columns, never to a RETURNS TABLE / OUT variable of the same
--    name (this was making report_bp_progress — and therefore
--    report_overview — raise "column reference is ambiguous").
--  * report_business_path: the CTE previously named `rows` (a keyword in
--    some grammar positions) is renamed `mrows`.
--  * report_overview: `generate_series(...) AS day` renamed to `gd`.
--  * Each jsonb RPC now traps errors and returns {"_error": <message>}
--    instead of aborting, so one failing section can't blank the page and
--    the message is visible in the UI.
-- Bodies are otherwise identical to 0041.
-- ============================================================
  ...
  case p_item.kind
    ...
    when 'monthly_goal' then
      return exists (select 1 from member_monthly_goals g
        where g.user_id = p_user and g.org_id = p_item.org_id and g.month = to_char(now(), 'YYYY-MM') and g.done);
    ...
    when 'three_month_goals' then
      select count(distinct g.month) into n from member_monthly_goals g where g.user_id = p_user and g.org_id = p_item.org_id;
      return n >= 3;
...
begin
  ...
  for m in
    select mp.user_id as uid, mp.current_rank_id as rid,
           coalesce(mp.started_at, 'epoch'::timestamptz) as started_at
  ...
  loop
    v_total := 0; v_done := 0; v_since := m.started_at;
    for it in
      select bi.* from business_path_items bi
      where bi.org_id = p_org and bi.rank_id = m.rid and bi.is_required
    loop
      v_total := v_total + 1;
      if report_bp_item_complete(it, m.uid, v_since) then v_done := v_done + 1; end if;
    end loop;
    user_id := m.uid;
    rank_id := m.rid;
...
begin
  ...
  with mem as (
    select m.user_id, m.status, m.joined_at from memberships m where m.org_id = p_org
  ),
  bp as materialized (select * from report_bp_progress(p_org)),
  ranks as (
    select r.id, r.name, r.order_index from business_path_ranks r where r.org_id = p_org and r.is_active
  ),
  recent_activity as (
  ...
  select jsonb_build_object(
    ...
    'bp_avg_percent', (select coalesce(round(avg(case when required_total > 0 then required_done::numeric / required_total * 100 else 0 end)), 0) from bp),
    'bp_ready', (select count(*) from bp where required_total > 0 and required_done >= required_total),
    'bp_near', (select count(*) from bp where required_total > 0 and required_done < required_total and required_done::numeric / required_total >= 0.75),
    'bp_pending_approvals', (select count(*) from business_path_item_progress where org_id = p_org and status = 'awaiting_approval'),
    'promotions_in_period', (select count(*) from member_rank_history where org_id = p_org and achieved_at >= p_start and achieved_at < p_end),
    'rank_distribution', (
    ...
    'income_period', (select coalesce(sum(amount), 0) from income_development_income_entries where org_id = p_org and earned_on >= p_start::date and earned_on < p_end::date),
    'income_period_prev', (select coalesce(sum(amount), 0) from income_development_income_entries where org_id = p_org and earned_on >= prev_start::date and earned_on < p_start::date),
    'income_earners', (select count(distinct user_id) from income_development_income_entries where org_id = p_org and earned_on >= p_start::date and earned_on < p_end::date),
    ...
    'assignments_pending_review', (select count(*) from coursework_submissions where org_id = p_org and status = 'submitted'),
    'onboarding_not_started', (
      select count(*) from memberships ms where ms.org_id = p_org and ms.status = 'active'
        and not exists (select 1 from onboarding_item_progress ip join onboarding_step_items si on si.id = ip.item_id where si.org_id = p_org and ip.user_id = ms.user_id)
    ),
    'inactive_7d', (select count(*) from mem where status = 'active' and user_id not in (select user_id from recent_activity)),
    'member_growth', (
      select coalesce(jsonb_agg(jsonb_build_object('date', d, 'total', running) order by d), '[]'::jsonb)
      from (
        select gd::date as d, (select count(*) from mem where joined_at::date <= gd::date) as running
        from generate_series(p_start, p_end - interval '1 day', interval '1 day') as gd
      ) g
    ),
    'promotions_series', (
      select coalesce(jsonb_agg(jsonb_build_object('date', d, 'count', c) order by d), '[]'::jsonb)
      from (select date_trunc('day', achieved_at)::date as d, count(*) as c from member_rank_history
            where org_id = p_org and achieved_at >= p_start and achieved_at < p_end group by 1) g
    ),
    'income_series', (
      select coalesce(jsonb_agg(jsonb_build_object('date', d, 'amount', a) order by d), '[]'::jsonb)
      from (select earned_on as d, sum(amount) as a from income_development_income_entries
            where org_id = p_org and earned_on >= p_start::date and earned_on < p_end::date group by 1) g
    )
  ) into result;

  return result;
exception when others then
  return jsonb_build_object('_error', sqlerrm, '_at', 'report_overview');
end;
$$;
...
begin
  ...
  select jsonb_build_object(
    'members', (select coalesce(jsonb_agg(jsonb_build_object(
        ...
        'required_total', required_total, 'required_done', required_done, 'percent', percent) order by percent desc), '[]'::jsonb) from mrows),
    'ready', (select coalesce(jsonb_agg(jsonb_build_object('user_id', user_id, 'name', full_name, 'rank', rank_name) order by full_name), '[]'::jsonb)
              from mrows where required_total > 0 and required_done >= required_total),
    'near', (select coalesce(jsonb_agg(jsonb_build_object('user_id', user_id, 'name', full_name, 'rank', rank_name, 'percent', percent) order by percent desc), '[]'::jsonb)
             from mrows where required_total > 0 and required_done < required_total and percent >= 75),
    'stalled', (select coalesce(jsonb_agg(jsonb_build_object('user_id', user_id, 'name', full_name, 'rank', rank_name, 'percent', percent) order by percent), '[]'::jsonb)
                from mrows where required_total > 0 and percent < 34),
    'pending_approvals', (
      select coalesce(jsonb_agg(jsonb_build_object('user_id', bpp.user_id, 'name', pr.full_name, 'item', bi.title, 'submitted_at', bpp.created_at) order by bpp.created_at), '[]'::jsonb)
      from business_path_item_progress bpp
      join business_path_items bi on bi.id = bpp.item_id
      join profiles pr on pr.id = bpp.user_id
      where bpp.org_id = p_org and bpp.status = 'awaiting_approval'
    ),
    'promotions', (
      select coalesce(jsonb_agg(jsonb_build_object('user_id', h.user_id, 'name', pr.full_name, 'rank', r.name, 'achieved_at', h.achieved_at, 'automatic', h.approved_by is null) order by h.achieved_at desc), '[]'::jsonb)
  ...
  return result;
exception when others then
  return jsonb_build_object('_error', sqlerrm, '_at', 'report_business_path');
end;
$$;
...
declare
  result jsonb;
  area text;
  areas text[] := array['onboarding','network_marketing','freelancing','personal_development','income_development'];
  area_json jsonb := '[]'::jsonb;
  actives uuid[];
begin
  if not has_org_role(p_org, array['admin']) then raise exception 'not authorized'; end if;
  ...
  foreach area in array areas[2:5] loop
    area_json := area_json || jsonb_build_object(
      ...
      'participants', (
        select count(distinct cip.user_id) from class_item_progress cip
      ...
      'modules_done', (select coalesce(sum(report_area_modules_done(p_org, area, u)), 0) from unnest(actives) u)
    );
  end loop;
  ...
  select jsonb_build_object(
    ...
    'onboarding', jsonb_build_object(
      ...
      'completed_members', (
        ...
        where (select count(*) from onboarding_step_items where org_id = p_org) > 0
          and not exists (select 1 from onboarding_step_items si where si.org_id = p_org
    ...
    'assessments', jsonb_build_object(
      ...
      'per_exam', (
        select coalesce(jsonb_agg(jsonb_build_object('exam_id', e.id, 'title', e.title, 'attempts', x.att, 'passed', x.pas,
          'pass_rate', case when x.att > 0 then round(x.pas::numeric / x.att * 100) else 0 end, 'avg_score', x.avg) order by x.att desc), '[]'::jsonb)
        from exams e
        join lateral (select count(*) as att, count(*) filter (where passed is true) as pas, round(avg(score_percent), 1) as avg
          from attempts a where a.exam_id = e.id and a.status = 'submitted' and a.is_guest = false) x on true
  ...
  return result;
exception when others then
  return jsonb_build_object('_error', sqlerrm, '_at', 'report_learning');
end;
$$;
...
begin
  ...
  select jsonb_build_object(
    ...
    'by_stage', (select coalesce(jsonb_object_agg(stage, c), '{}'::jsonb) from (select stage, count(*) c from network_marketing_contacts where org_id = p_org group by stage) s),
    'prospects_added', (select count(*) from network_marketing_contacts where org_id = p_org and created_at >= p_start and created_at < p_end),
    ...
    'members_joined_via_sponsor', (
      select count(*) from profiles p join memberships m on m.user_id = p.id
      where m.org_id = p_org and m.status = 'active' and p.sponsor_member_id is not null and m.joined_at >= p_start and m.joined_at < p_end
    ),
    'top_builders', (
      select coalesce(jsonb_agg(jsonb_build_object('user_id', uid, 'name', nm, 'directs', c) order by c desc), '[]'::jsonb)
      from (
        select sp.id as uid, sp.full_name as nm, count(*) as c
        from profiles ch
        ...
        group by sp.id, sp.full_name order by c desc limit 10
      ) t
    )
  ) into result;
  return result;
exception when others then
  return jsonb_build_object('_error', sqlerrm, '_at', 'report_network');
end;
$$;
...
begin
  ...
  from (
    select jsonb_build_object(
      ...
      'bp_avg', (select coalesce(round(avg(case when b.required_total > 0 then b.required_done::numeric / b.required_total * 100 else 0 end)), 0)
                 from group_members gm join bp b on b.user_id = gm.user_id where gm.group_id = g.id),
      'network_growth', (select count(*) from group_members gm
        join profiles ch on ch.sponsor_member_id = gm.user_id
        join memberships m on m.user_id = ch.id and m.org_id = p_org and m.status = 'active'
        where gm.group_id = g.id and m.joined_at >= p_start and m.joined_at < p_end),
      'income', (select coalesce(sum(e.amount), 0) from group_members gm
  ...
  return result;
exception when others then
  return jsonb_build_object('_error', sqlerrm, '_at', 'report_teams');
end;
$$;
...
begin
  ...
  select jsonb_build_object(
    ...
    'by_source', (
      ...
      from (select source, sum(amount) amt from income_development_income_entries where org_id = p_org and earned_on >= p_start::date and earned_on < p_end::date group by source) s
    ),
    'per_member', (
      select coalesce(jsonb_agg(jsonb_build_object('user_id', uid, 'name', nm, 'amount', amt) order by amt desc), '[]'::jsonb)
      from (select p.id uid, p.full_name nm, sum(e.amount) amt from income_development_income_entries e join profiles p on p.id = e.user_id
            where e.org_id = p_org and e.earned_on >= p_start::date and e.earned_on < p_end::date group by p.id, p.full_name) m
    ),
    'milestones', (
  ...
  return result;
exception when others then
  return jsonb_build_object('_error', sqlerrm, '_at', 'report_income');
end;
$$;
