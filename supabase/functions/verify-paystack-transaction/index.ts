...
// Paystack's inline popup reports success, so the Billing page can flip to
// "active" without waiting on webhook delivery (which is usually fast, but
// not guaranteed to beat the browser's own next render). The
// paystack-webhook function is the durable path that still fires
