...
### Finance Phase 1 — provider-ready payout control (`0065_finance_phase1.sql`)
...
**Phase 2 (not built).** `createTransferRecipient` on account verify, `finance-payout` + `finance-transfer-webhook` edge functions, `payout_attempts` automation + retry, `provider_webhook_events` dedupe, scheduled reconciliation — all gated on confirmed provider capability + the office's own provider account. Phase 3 (collection / subaccounts / DVA / direct payments) is untouched.

**Known limits (Phase 1).** 0043's order/settlement RPCs still `fin_assert_admin` (only the withdrawal workflow is grant-aware), so a non-admin `verify_settlement` grant can't yet record earnings from the UI (the "Record earning" button is admin-only); `daily_payout_limit` / per-request limit are enforced only when the request currency matches `base_currency`; no encryption-at-rest for `account_number` yet (masking + recipient-code migration is Phase 2); not runtime-tested against a live provider.
