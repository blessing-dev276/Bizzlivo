...
### Finance Phase 3a — payout SLA, mandatory external reconciliation, auto-reversal (`0077_finance_sla.sql`)

Time-based safety rails around a withdrawal that is mid-payment. Nothing here changes money storage or the non-custodial model.

- **New config** on `organization_finance_config` (all `finance_update_config`-editable, audited): `payout_sla_hours` (default 24, 1–720), `require_external_reconciliation` (default **on**), `auto_reversal_hours` (nullable — **off** by default; automated provider transfers only).
- **`finance_sla_sweep()`** — `SECURITY DEFINER`, `service_role`-only, scheduled by `pg_cron` (`finance-sla-sweep`, `*/15 * * * *`, pure SQL — no edge function). Per finance-enabled org, over withdrawals in `authorized_for_payment` / `payment_recorded`:
  1. **SLA breach** — time-in-state (`payment_recorded_at` / `authorized_at`) past `payout_sla_hours` → upserts a `finance_reconciliation_flags` row `kind='payout_sla_breach'`, notifies the member once (`notify`, dedupe `wd-sla:<id>`, "taking longer than usual … stays reserved for you"), one `finance_events` `withdrawal_sla_breach` audit row. The amount is **not** touched.
  2. **Auto-reversal** (only if `auto_reversal_hours` set) — a `provider_transfer` payment still `recorded` (no webhook) past the window → payment row → `failed` (`failure_code='auto_reversal_timeout'`), a `withdrawal_release` ledger row returns the reservation to Available, withdrawal → `failed` with `active_payment_id = null`, `finance_events` `withdrawal_auto_reversed`, member notified ("returned to your available balance — you can request it again"), and a `finance_reconciliation_flags` `auto_reversed_payout` row. External/office payments are never auto-reversed.
- **Mandatory external reconciliation** — `finance_record_withdrawal_payment` now finalizes to `paid` immediately only when `require_payment_confirmation = false` **and** the channel is not external-under-`require_external_reconciliation`; an office-operated payment therefore always needs a second capable confirmer.
- **Double-pay guard** — `_fin_reservation_live(withdrawal)` (net `affects_balance` ledger `< 0`). `finance_record_withdrawal_payment` and `finance_transfer_begin` refuse a withdrawal whose reservation was already released (e.g. auto-reversed) — the member must request anew.
- **UI** — Settings → Finance → Withdrawal Rules gains SLA hours, auto-reversal window, and the "always reconcile external payments" toggle; `finance_org_overview.needs_attention.payout_sla_breaches` surfaces on the Finance Overview → jumps to Reconciliation; member Wallet shows an auto-reversed withdrawal as **Returned** (via `isAutoReversed`), not "Processing".

**Not built.** Phase 3 (office collection accounts / subaccounts / splits / DVA / direct customer payments) is untouched; `finance-payout` / `finance-transfer-webhook` are not deployed until Transfers is confirmed on a real provider account.
